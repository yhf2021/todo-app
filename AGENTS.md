...
```
┌──────────────────────────────────────────┐
│  ✨ 我的待办清单     2026-05-11     yyy  退出 │  ← 标题 + 日期(居中) + 用户名 + 退出按钮
├──────────────────────────────────────────┤
│  [今天要做什么？________________] [🔴高] │  ← 输入框 + 优先级选择
│  [➕ 添加                               ]│
├──────────────────────────────────────────┤
│  ⚠️ 请输入待办内容                        │  ← 空输入错误提示（红色，2秒消失）
├──────────────────────────────────────────┤
│  待完成 [3]                    已完成 [2] │  ← 统计数量
├──────────────────────────────────────────┤
│  ☐ 🔴 买牛奶                    ✏️  ×   │  ← 高优先级（红色左边框）
│  ☑ 🟡 写作业  ~~~删除线~~~      ✏️  ×   │  ← 已完成（灰色+删除线）
│  ☐ 🟢 去跑步                    ✏️  ×   │  ← 低优先级（绿色左边框）
│  ☐ 🟡 编辑中... [____]     💾  ✏️  ×   │  ← 编辑模式（黄色背景）
├──────────────────────────────────────────┤
│        [🗑️ 清空已完成]  [清空全部]       │  ← 底部操作按钮
└──────────────────────────────────────────┘
